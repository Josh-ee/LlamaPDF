...
CHUNK_SIZE = 512 # Determined by the embedding model
CHUNK_OVERLAP = 20
...
# Run Code: "chainlit run PDF_Pal5.py" in this directory
...
class ChatBot:
    def __init__(self, memory_prompt, base_answer_prompt, rag_prompt, base_prompt):
...
The user will ask you about one or more academic paper for which you have the summaries
...
Concise Answer based on summaries: """)
...
You are Paper Pilot, an AI Assistant for understanding academic papers, assume all questions are about the uploaded paper\n
...
You are a friendly and helpful PDF bot called 'Paper Pilot'
You are only allowed to chat about your capabilities and to assist the user with uploading PDFs.
                             
There are two options to upload PDFs.
1. Type "Upload" into the chat.
2. Paste a arXiv link into the chat

The user can upload 1 PDF at at time.
Please let the user know that the more PDFs they upload, the longer it will take to process  
                                                      
USER QUESTION: {question}

Concise Answer: """)

pdf_chat_bot = ChatBot(MEM_PROMPT, HYDE_PROMPT, RAG_PROMPT, BASE_PROMPT)
print("Model Loaded")
...
def extract_first_five_pages(pdf_path):
    # Open the PDF file
    ...
    # Extract text from the first five pages
    ...
    message = client.messages.create(
        ...
        max_tokens=2000,
        temperature=0.0,
        system="Please summarize this paper, start by repeating the title and authors, them give your 2 paragraph summary:",
        stream=False,
        messages=[
...
async def factory():
    ...
    Settings.llm = pdf_chat_bot.llm
    
    Settings.embed_model = pdf_chat_bot.embedding_model
    # Settings.node_parser = SentenceSplitter(chunk_size=512, chunk_overlap=20)
    Settings.num_output = 512
    Settings.context_window = 3900

    pdf_chat_bot.new_chat()

    

    cl.user_session.set("llm", pdf_chat_bot.llm)
...
def check_arxiv(question):
    ...
    if match:
        ...
        arxiv_id = None
        check_arxiv_url_included = False
        return False, None
    ...
    if arxiv_id is not None:
        ...
        check_arxiv_url_included = True 
        # print(paper.title)
        return True, paper_path

@cl.on_message
async def main(message: cl.Message):
    # print(pdf_chat_bot.summaries)
    ...
    if match:
        ...
        load_str = "Paper found, please wait while I download and process...\n"
    ...
    if arxiv_id is not None:
        ...
        search = arxiv.Search(id_list=[arxiv_id])
        paper = next(arxiv.Client().results(search))
        arxiv_paper_path = paper.download_pdf()
        check_arxiv_url_included = True 


    if arxiv_paper_path:
        # Replace 'your_pdf_file.pdf' with the path to your PDF file
        pdf_text = extract_first_five_pages(arxiv_paper_path)
    ...
    if check_upload_keyword or check_arxiv_url_included:
        ...
        if check_upload_keyword:
            ...
            while files == None:
                ...
                content="Please upload a pdf file to begin!", accept=["pdf"], max_size_mb=10
            ).send()
        ...
        # sync
        documents = await parser.aload_data(pdf_file.path if arxiv_paper_path is None else arxiv_paper_path)
        # documents = parser.load_data(pdf_file.path if arxiv_paper_path is None else arxiv_paper_path)

        summary = extract_first_five_pages(pdf_file.path if arxiv_paper_path is None else arxiv_paper_path)
        pdf_chat_bot.summaries += f'{summary}\nPaper End\n'
        # print(documents)
        ...
        # index = VectorStoreIndex.build_index_from_nodes(nodes=nodes, service_context=Settings)
        ...
        ty_msg = f'\n\nProcessing Done! Thank you for your patience\n{summary}'
        for char in str(ty_msg):
    ...
    if pdf_chat_bot.question_count != 0:
        ...
        rephrase_prompt = pdf_chat_bot.memory_prompt.format(chat_history = pdf_chat_bot.chat_history, question=question)
        question = await cl.make_async(llm.complete)(rephrase_prompt)
        question = (question.text).strip()

    if pdf_chat_bot.DOCS_LOADED == False:

        base_question = pdf_chat_bot.base_prompt.format(question = question)        

        response_message = cl.Message(content="")
        for response in await cl.make_async(llm.stream_complete)(base_question):
        ...
        full_question = pdf_chat_bot.base_answer_prompt.format(question = question, summaries = pdf_chat_bot.summaries)
        ...
        rag_question = pdf_chat_bot.rag_prompt.format(query_str = question, context_str = sorted_sources_text )        
    ...
    pdf_chat_bot.chat_history.append(
            ChatMessage(
    ...
    new_row_df.to_csv(pdf_chat_bot.out_file, mode='a', header=False, index=False)

    pdf_chat_bot.question_count += 1
